...
require("lazy").setup({
	...
	{ -- Plugin that allows you to run queries within nvim
		"nanotee/sqls.nvim",
		ft = { "sql", "mysql", "psql" },
		config = function()
			-- New Neovim 0.11+ Way
		...
		config = function()
			...
			vim.api.nvim_create_autocmd("LspAttach", {
				...
				callback = function(event)
					...
					--  SQLS: Add key binds for sqls so that we can run queries in nvim
					if client.name == "sqls" then
						map("<leader>rq", "<cmd>SqlsExecuteQuery<CR>", "[R]un [Q]uery", "n")
						map("<leader>rq", "<cmd>SqlsExecuteQueryVertical<CR>", "[R]un [Q]uery (Vertical)", "v")
						map("<leader>rs", "<cmd>SqlsSwitchDatabase<CR>", "[R]esource [S]witch DB", "n")
					end
